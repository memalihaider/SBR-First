...
import { Briefcase, Users, Clock, CheckCircle, AlertCircle, DollarSign, FileText, Link, Plus, Edit, Trash2, ExternalLink, User, Filter } from 'lucide-react';
...
export default function AdminProjectsPage() {
  ...
  // Subscribe to projects collection on mount - REAL TIME
  useEffect(() => {
    const fetchProjectsRealtime = () => {
      try {
  ...
  // Fetch employees from employeeList collection - REAL TIME
  useEffect(() => {
  ...
  // Filter projects by selected employee
  const filteredProjects = projects.filter(project => {
    if (selectedEmployee === 'all') return true;
    
    // Check if employee is project manager
    if (project.projectManager === selectedEmployee) return true;
    
    // Check if employee is in team members
    if (project.teamMembers && project.teamMembers.includes(selectedEmployee)) return true;
  ...
  const calculateStats = () => {
    const projectsToCalculate = selectedEmployee === 'all' 
      ? projects 
      : filteredProjects;
    
    return {
      totalProjects: projectsToCalculate.length.toString(),
      activeProjects: projectsToCalculate.filter(p => p.status === 'active').length.toString(),
      completedProjects: projectsToCalculate.filter(p => p.status === 'completed').length.toString(),
      planningProjects: projectsToCalculate.filter(p => p.status === 'planning').length.toString()
    };
  };
  ...
  const projectStats = [
    { title: 'Total Projects', value: stats.totalProjects, change: '+' + Math.floor(parseInt(stats.totalProjects) * 0.08), icon: Briefcase, color: 'blue' },
    { title: 'Active Projects', value: stats.activeProjects, change: '+5', icon: Clock, color: 'green' },
    { title: 'Completed', value: stats.completedProjects, change: '+12', icon: CheckCircle, color: 'purple' },
    { title: 'Planning', value: stats.planningProjects, change: '+3', icon: AlertCircle, color: 'yellow' }
  ];
  ...
  async function addProjectToFirestore(projectData: any) {
    ...
    toSave.documents = toSave.documents || [];
  ...
  const handleAddProject = async () => {
    try {
      ...
      const toCreate = {
        ...
        documentationLink: newProject.documentationLink || '',
        status: 'planning',
        completionPercentage: 0,
  ...
  const handleManageProject = async (project: any) => {
    ...
    try {
      ...
      } else if (manageAction === 'add_document') {
      ...
      setDocumentData({ name: '', link: '', type: 'documentation' });
      setNewTeamMember('');
          ...
          <div>
            ...
            <p className="text-red-100 mt-1 text-lg">
              {selectedEmployee === 'all' 
                ...
                : `Projects for ${getSelectedEmployeeName()}`
              }
            </p>
          </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {employees.find(e => e.name === selectedEmployee)?.department || 'No department'} • 
                    {employees.find(e => e.name === selectedEmployee)?.position || 'No position'}
                  </p>
                  <div className="flex items-center gap-4 mt-2">
                    <span className="text-sm text-gray-700">
                      <span className="font-bold">{filteredProjects.length}</span> projects assigned
                    </span>
                    <span className="text-sm text-gray-700">
                      Project Manager in <span className="font-bold">
                        {filteredProjects.filter(p => p.projectManager === selectedEmployee).length}
                      </span> projects
                    </span>
                    <span className="text-sm text-gray-700">
                      Team Member in <span className="font-bold">
                        {filteredProjects.filter(p => p.teamMembers?.includes(selectedEmployee)).length}
                      </span> projects
                    </span>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-gray-600 font-medium">
                {filteredProjects.length} projects {selectedEmployee !== 'all' ? 'assigned' : 'across all departments'}
              </CardDescription>
            </div>
          ...
          <div className="space-y-4">
            {filteredProjects.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 max-w-md mx-auto">
                  {selectedEmployee === 'all' 
                    ...
                    : `No projects found for ${getSelectedEmployeeName()}.`
                  }
                </p>
              </div>
            ) : (
              filteredProjects.slice(0, 15).map((project: any) => {
                ...
                // Check if selected employee is project manager or team member
                const isProjectManager = selectedEmployee !== 'all' && project.projectManager === selectedEmployee;
                const isTeamMember = selectedEmployee !== 'all' && project.teamMembers?.includes(selectedEmployee);
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          {isProjectManager && (
                            <Badge variant="default" className="bg-green-600">Project Manager</Badge>
                          )}
                          {isTeamMember && (
                            <Badge variant="secondary" className="bg-blue-600">Team Member</Badge>
                          )}
                        </div>
                              ...
                              <div className="space-y-6">
                                <Tabs defaultValue="overview" className="w-full">
                                  ...
                                  <TabsContent value="overview" className="space-y-4">
                                    ...
                                    {/* Documentation Link in Overview */}
                                    {project.documentationLink && (
                                      <div>
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                      ...
                                      <Card className="bg-white border border-gray-200">
                                        <CardContent className="pt-4">
                                          <div className="text-center">
                                            <div className="text-2xl font-bold text-green-600">{project.completionPercentage}%</div>
                                      ...
                                      <Card className="bg-white border border-gray-200">
                                        <CardContent className="pt-4">
                                          <div className="text-center">
                                            <div className="text-xl font-bold text-blue-600">{project.projectManager || '—'}</div>
                              ...
                              <div className="space-y-4">
                                <Tabs value={manageAction} onValueChange={(value) => setManageAction(value as any)} className="w-full">
                                  <TabsList className="grid w-full grid-cols-5">
                                    ...
                                    <TabsTrigger value="add_milestone">Milestones</TabsTrigger>
