...
import { MoreVertical, Eye, Edit, Trash2, Calendar, User, Clock, FileText , Users, CheckCircle, PlayCircle, PauseCircle, Filter } from 'lucide-react';
import { 
  DropdownMenu, 
  DropdownMenuContent, 
  DropdownMenuItem, 
...
export default function ProjectTasksPage() {
  ...
  // Filtered tasks - Employee filter + status filter + search
  const filteredTasks = tasks.filter(task => {
    // Employee filter
    const matchEmployee = selectedEmployee === 'all' || task.assignee === selectedEmployee;
  ...
  // Stats - Filtered by selected employee
  const calculateStats = () => {
    const tasksToCalculate = selectedEmployee === 'all' 
      ? tasks 
      : tasks.filter(task => task.assignee === selectedEmployee);
    
    return {
      totalTasks: tasksToCalculate.length,
      pendingTasks: tasksToCalculate.filter(t => t.status === 'pending').length,
      inProgressTasks: tasksToCalculate.filter(t => t.status === 'in progress').length,
      completedTasks: tasksToCalculate.filter(t => t.status === 'completed').length
    };
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header + Employee Filter + Create Task */}
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {selectedEmployee === 'all' 
              ...
              : `Tasks assigned to ${getSelectedEmployeeName()}`
            }
          </p>
        </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 max-w-md mx-auto">
                {selectedEmployee === 'all' 
                  ...
                  : `No tasks found for ${getSelectedEmployeeName()}.`
                }
              </p>
            </div>
          ) : (
