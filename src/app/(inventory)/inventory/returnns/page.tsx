...
export default function AdminInventoryReturnsPage() {
      ...
      try {
        ...
        const returnsUnsubscribe = onSnapshot(returnsQuery, (snapshot) => {
          ...
          snapshot.forEach((doc) => {
            const data = doc.data();
            const returnDate = data.createdAt?.toDate?.() || new Date();
            
            returnsData.push({
              id: doc.id,
              returnNumber: data.returnNumber || '',
              type: data.type || 'customer_return',
              status: data.status || 'pending',
              priority: data.priority || 'normal',
              totalQuantity: data.totalQuantity || 0,
              totalValue: data.totalValue || 0,
              requestedDate: returnDate,
              actualReturnDate: data.actualReturnDate?.toDate?.() || undefined,
              returnMethod: data.returnMethod || 'drop_off',
              customerNotes: data.customerNotes || '',
              trackingNumber: data.trackingNumber || '',
              resolutionNotes: data.resolutionNotes || '',
              refundAmount: data.refundAmount || 0,
              restockingFee: data.restockingFee || 0,
              finalRefundAmount: data.finalRefundAmount || 0,
              items: data.items || [],
          ...
          // Remove duplicates
          const uniqueReturns = returnsData.filter((returnItem, index, self) =>
            index === self.findIndex(r => r.id === returnItem.id)
          );
          
          setReturns(uniqueReturns);

          // Calculate stats
          const total = uniqueReturns.length;
          const pending = uniqueReturns.filter(r => r.status === 'pending').length;
          const approved = uniqueReturns.filter(r => r.status === 'approved').length;
          const processed = uniqueReturns.filter(r => 
            ['refunded', 'replaced', 'received', 'inspected'].includes(r.status)
          ).length;
          const totalValue = uniqueReturns.reduce((sum, r) => sum + r.totalValue, 0);
          
          setStats({ total, pending, approved, processed, totalValue });
        });
  ...
  const handleSaveCreateReturn = async () => {
    try {
      ...
      // Create return in Firebase
      const returnData = {
        returnNumber,
        type: createForm.type,
        referenceType: createForm.referenceType === 'none' ? null : createForm.referenceType,
        referenceId: createForm.referenceType === 'none' ? null : createForm.referenceId,
        customerId: createForm.type === 'customer_return' ? createForm.customerId : null,
        vendorId: createForm.type === 'vendor_return' ? createForm.vendorId : null,
        items: returnItems,
        totalQuantity,
        totalValue,
  ...
  const handleSaveEditReturn = async () => {
    ...
    try {
      await updateDoc(doc(db, 'returns', selectedReturn.id), {
        priority: editForm.priority,
        returnMethod: editForm.returnMethod,
        customerNotes: editForm.customerNotes || null,
        updatedAt: Timestamp.now()
      });
  ...
  const handleDeleteReturn = async (returnRequest: ReturnRequest) => {
    ...
    try {
      // First, add back stock for all items
      ...
      await deleteDoc(doc(db, 'returns', returnRequest.id));
      toast.success(`Return ${returnRequest.returnNumber} deleted and stock restored successfully!`);
    } catch (error) {
      console.error('Error deleting return:', error);
      toast.error('Failed to delete return');
    }
  ...
  const handleSaveProcess = async () => {
    ...
    try {
      ...
      if (processForm.resolutionNotes) {
        updates.resolutionNotes = processForm.resolutionNotes;
      }

      if (processForm.trackingNumber) {
        updates.trackingNumber = processForm.trackingNumber;
      }
      ...
      if (processForm.status === 'received' || processForm.status === 'replaced') {
        ...
        if (processForm.status === 'received') {
          updates.actualReturnDate = Timestamp.now();
        }
      }

      await updateDoc(doc(db, 'returns', selectedReturn.id), updates);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {selectedReturn.finalRefundAmount > 0 && (
                      <div className="flex justify-between">
                        <span>Final Refund:</span>
                        <span className="font-medium text-green-600">${selectedReturn.finalRefundAmount.toLocaleString()}</span>
                      </div>
